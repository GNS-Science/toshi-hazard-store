...
def all_locations() -> List[CodedLocation]:
    ...
    locations_nz35 = [CodedLocation(loc['latitude'], loc['longitude'], RESOLUTION) for loc in LOCATIONS_BY_ID.values()]

    grid = RegionGrid[SITE_LIST]
    grid_locs = grid.load()
    locations_grid = [CodedLocation(*loc, RESOLUTION) for loc in grid_locs]
...
def concat_hazard_curves(hc1: DataFrame, hc2: DataFrame) -> DataFrame:
    return pd.concat((hc1, hc2), ignore_index=True).drop_duplicates(subset=['lat', 'lon', 'imt', 'agg', 'level'])
...
def hazard_from_archive(
    hazard_id: str, vs30: int, locations: List[CodedLocation], imts: List[str], aggs: List[str]
) -> DataFrame:
...
def get_hazard(
    hazard_id: str, locs: List[CodedLocation], vs30: int, imts: List[str], aggs: List[str], no_archive: Any = False
) -> DataFrame:
