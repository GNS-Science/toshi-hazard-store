...
while IFS= read -r id; do
    ...
    echo "Running ths_rlz_import producers $id"
    echo "Running ths_rlz_import producers $id" >> "$LOG_DIR/producers.log"
    echo "============================" >> "$LOG_DIR/producers.log"
    {
        start_time=$(date +%s)  # Record the start time
        $time -f "Time taken for ths_rlz_import %E" ths_rlz_import producers "$id" NZSHM22 -W "$WORKDIR" --verbose >> "$LOG_DIR/producers.log" 2>&1 | grep 'Time taken for' &
        pid=$!
        wait $pid # Wait until the command is done.
        if [ $? -ne 0 ]; then
            echo "Error: ths_rlz_import producers failed with id '$id'. Exiting loop."
            break
        fi        
    ...
    echo "Completed ths_rlz_import producers $id" >> "$LOG_DIR/producers.log"
    echo "==============================" >> "$LOG_DIR/producers.log"

    # Call the second program with the current id as an argument and measure time taken
    echo "Running ths_rlz_import extract $id"
    echo "Running ths_rlz_import extract $id" >> "$LOG_DIR/extract.log"
    echo "============================" >> "$LOG_DIR/extract.log"
    {
        start_time=$(date +%s)  # Record the start time
        $time -f "Time taken for ths_rlz_import %E" ths_rlz_import extract "$id" NZSHM22 -W "$WORKDIR" -O "$TARGET/$id" -CID --verbose >> "$LOG_DIR/extract.log" 2>&1 | grep 'Time taken for' &
        pid=$!
        wait $pid # Wait until the command is done.
        if [ $? -ne 0 ]; then
    ...
    echo "Completed ths_rlz_import extract $id" >> "$LOG_DIR/extract.log"
    echo "==============================" >> "$LOG_DIR/extract.log"

    # Call the third program with the current id as an argument and measure time taken
    echo "Running ths_rlz_sanity count-rlz $id"
    echo "Running ths_rlz_sanity count-rlz $id" >> "$LOG_DIR/sanity.log"
    echo "============================" >> "$LOG_DIR/sanity.log"
    {
        start_time=$(date +%s)  # Record the start time
        $time -f "Time taken for ths_rlz_sanity %E" ths_rlz_sanity count-rlz "$TARGET/$id" -x --expected-rlzs $EXPECTED_RLZ_COUNT >> "$LOG_DIR/sanity.log" 2>&1 | grep 'Time taken for' &
        pid=$!
        wait $pid # Wait until the command is done.
        if [ $? -ne 0 ]; then
            echo "Error: ths_rlz_sanity count-rlz failed with id '$id'. Exiting loop."
            break
        fi        
        end_time=$(date +%s)  # Record the end time
    } &> /dev/null
    ...
    echo "Completed ths_rlz_sanity count-rlz $id" >> "$LOG_DIR/sanity.log"
    echo "==============================" >> "$LOG_DIR/sanity.log"
