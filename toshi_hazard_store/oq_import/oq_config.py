...
__all__ = [
    ...
    "download_artefacts",
    "process_hdf5",
]  # constrain what gets imported with `import *.`
...
def download_artefacts(
    ...
    _save_api_file(
        subtask_folder / TASK_ARGS_JSON,
        hazard_task_detail["hazard_solution"]["task_args"]["file_url"],
    )
...
def process_hdf5(
    ...
    hdf5_files = list(
        itertools.chain(
            subtask_folder.glob("calc_[0-9].hdf5"),
            subtask_folder.glob("calc_[0-9][0-9].hdf5"),
        )
    )
